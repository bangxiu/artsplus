...
//自动切换tab
Element.prototype.zidong=function(ele){
			var $ulchild=this.childNodes;
			var $conchild=ele.childNodes;
			...
			function showtab(_now){
				 for(var i=0;i<$ulchild.length;i++){
				 	if(i==_now){
				 		 $ulchild[_now].setAttribute('class','act');
						 $conchild[_now].style.display="block";
						}else{
						 	$ulchild[i].removeAttribute('class');
						 	$conchild[i].style.display="none";
						}
				 }
			}
...
//依赖hasClass	
//参数：cn	   		   		   
Element.prototype.toggleClass = function (cn) {
            if(this.hasClass(cn)){
                this.removeClass(cn);
            }
            else{
